...
public class Tour {
    ...
    public void insertNearest(Point p) {
        ...
        //achar o nó anterior ao nearest
        Node prev = start;
        while (prev.next != nearest) {
            prev = prev.next;
        }

        //testar duas posições possíveis
        double costAfter =
                nearest.p.distanceTo(p) +
                        p.distanceTo(nearest.next.p) -
                        nearest.p.distanceTo(nearest.next.p);

        double costBefore =
                prev.p.distanceTo(p) +
                        p.distanceTo(nearest.p) -
                        prev.p.distanceTo(nearest.p);
        ...
        if (costAfter < costBefore) {
            ...
            newNode.next = nearest;
            prev.next = newNode;

            //se inseriu antes do start, atualiza
            if (nearest == start) {
                start = newNode;
            }
        }
